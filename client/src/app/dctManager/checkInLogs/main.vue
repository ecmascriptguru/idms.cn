      ...
      */
      remove(item) {
        this.$http.delete(`dct/buildings/${item.id}`).then(() => {
          /**
          * On success fetch a new set of CheckInLogs
    ...
    */
    computed: {
      ...mapState({
        ...
        pagination: state => state.DctManager.CheckInLogs.pagination,
        list: state => state.DctManager.CheckInLogs.list,
      }),
      buildings() {
        return this.list
      },

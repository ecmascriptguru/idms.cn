      ...
      */
      remove(item) {
        this.$http.delete(`dct/buildings/${item.id}`).then(() => {
          /**
          * On success fetch a new set of Certifications
    ...
    */
    computed: {
      ...mapState({
        ...
        pagination: state => state.DctManager.Certifications.pagination,
        list: state => state.DctManager.Certifications.list,
      }),
      buildings() {
        return this.list
      },

      ...
      */
      remove(item) {
        this.$http.delete(`dct/buildings/${item.id}`).then(() => {
          /**
          * On success fetch a new set of OwnerCards
    ...
    */
    computed: {
      ...mapState({
        ...
        pagination: state => state.DctManager.OwnerCards.pagination,
        list: state => state.DctManager.OwnerCards.list,
      }),
      buildings() {
        return this.list
      },

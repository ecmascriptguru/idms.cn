  ...
  export default {
    ...
    */
    ...
    methods: {
      ...
      create() {
        this.$router.push({ name: 'opManager.deviceAdvs.new', query: { page: this.currentPage } })
      },
      hide() {
        this.$router.push({ name: 'opManager.deviceAdvs.index', query: { page: this.currentPage } })
      },
      /**
      * Brings actions from Vuex to the scope of
      * this component
      */
      ...mapActions(['deviceAdvsSetData', 'setFetching']),
      ...
      */
      fetchPaginated() {
        ...
        */
        this.$http.get(`oca/device-advs?page=${this.currentPage}`).then(({ data }) => {
          /**
          * Vuex action to set pagination object in
          * the Vuex OpratingCompany module
          */
          this.deviceAdvsSetData({
            list: data.data,
      ...
      * return a paginated set of deviceAdvs
      * this one returns the full set, which
      * is used by other components in the app.
      */
      fetchFullList() {
        this.setFetching({ fetching: true })
        this.$http.get('oca/device-advs/full-list').then(({ data }) => {
          /**
      ...
      */
      navigate(obj) {
        ...
        */
        this.$router.push({ name: 'opManager.deviceAdvs.index', query: { page: obj.page } })
      ...
      */
      askRemove(item) {
        swal({
          title: 'Are you sure?',
          text: `Device Advertisement ${item.name} will be permanently removed.`,
          type: 'warning',
      ...
      */
      remove(item) {
        this.$http.delete(`oca/device-advs/${item.id}`).then(() => {
          /**
          ...
          */
          swal('Done!', 'Device Advertisement removed.', 'success')
    ...
    */
    computed: {
      ...mapState({
        ...
        pagination: state => state.OpManager.DeviceAdvs.pagination,
        list: state => state.OpManager.DeviceAdvs.list,
      }),
      deviceAdvs() {
        return this.list
      },
      ...
      isFormVisible() {
        return this.$route.name === 'opManager.deviceAdvs.new' || this.$route.name === 'opManager.deviceAdvs.edit'
      },
    },
    /**
    ...
    */
    beforeRouteLeave(to, from, next) {
      ...
      this.$bus.$off('deviceAdv.created')
      this.$bus.$off('deviceAdv.updated')
    ...
    mounted() {
      ...
      */
      this.$bus.$on('deviceAdv.created', this.fetch)
      this.$bus.$on('deviceAdv.updated', this.fetch)
      /**

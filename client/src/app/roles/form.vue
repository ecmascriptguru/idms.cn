
<script>
  import { mapActions } from 'vuex'

  export default {
    name: 'CcRolesForm',

    /**
    * Component's local state
    */
    data() {
      return {
        role: {
          id: 0,
          name: '',
        },
      }
    },

    /**
    * Fetch role when component is first mounted
    */
    mounted() {
      this.fetch()
    },

    /**
    * Also fetch role every time route changes
    */
    watch: {
      $route: 'fetch',
    },

    /**
    * Determines based on the presence of
    * role id if the current actions
    * is editing instead of creating.
    */
    computed: {
      isEditing() {
        return this.role.id > 0
      },
      isValid() {
        this.resetMessages()
        if (this.role.name === '') {
          this.setMessage({ type: 'error', message: ['Please fill role name'] })
          return false
        }
        return true
      },
    },

    methods: {
      ...mapActions(['setFetching', 'resetMessages', 'setMessage']),

      /**
      * If there's an ID in the route params
      * then use it to fetch the role
      * from the server
      */
      fetch() {
        this.$refs.firstInput.focus()

        const id = this.$route.params.id
        /**
        * This same component is used for create
        * and update so we have to check if
        * ID is not undefined.
        */
        if (id !== undefined) {
          /**
          * Fetch the role from the server
          */
          this.setFetching({ fetching: true })
          this.$http.get(`roles/${id}`).then((res) => {
            const { id: _id, name } = res.data.data // http://wesbos.com/destructuring-renaming/
            this.role.id = _id
            this.role.name = name
            this.setFetching({ fetching: false })
          })
        }
      },
      submit() {
        /**
        * Pre-conditions are met
        */
        if (this.isValid) {
          /**
          * Shows the global spinner
          */
          this.setFetching({ fetching: true })

          if (this.isEditing) {
            this.update()
          } else {
            this.save()
          }
        }
      },
      save() {
        this.$http.post('roles', { name: this.role.name }).then(() => {
          /**
          * This event will notify the world about
          * the role creation. In this case
          * the Role main component will intercept
          * the event and refresh the list.
          */
          this.$bus.$emit('role.created')

          /**
          * Hides the global spinner
          */
          this.setFetching({ fetching: false })

          /**
          * Sets the global feedback message
          */
          this.setMessage({ type: 'success', message: 'New role was created' })

          /**
          * Resets component's state
          */
          this.reset()
        })
      },
      update() {
        this.$http.put(`roles/${this.role.id}`, this.role).then(() => {
          /**
          * This event will notify the world about
          * the role creation. In this case
          * the Role main component will intercept
          * the event and refresh the list.
          */
          this.$bus.$emit('role.updated')

          /**
          * Hides the global spinner
          */
          this.setFetching({ fetching: false })

          /**
          * Sets the global feedback message
          */
          this.setMessage({ type: 'success', message: 'Role was updated' })
        })
      },
      reset() {
        this.role.id = 0
        this.role.name = ''
      },
    },
  }
</script>

<template>
  <form @submit.prevent="submit" class="well">
    <div class="form-group">
      <label for="name" class="control-label">Role Name</label>
      <input ref="firstInput" type="text" id="name" class="form-control" v-model="role.name">
    </div>
    <button class="btn btn-primary btn-xs" type="submit">Salvar</button>
  </form>
</template>

  ...
  export default {
    ...
    */
    ...
    methods: {
      ...
      create() {
        this.$router.push({ name: 'admin.checkInTypes.new', query: { page: this.currentPage } })
      },
      hide() {
        this.$router.push({ name: 'admin.checkInTypes.index', query: { page: this.currentPage } })
      },
      /**
      * Brings actions from Vuex to the scope of
      * this component
      */
      ...mapActions(['checkInTypesSetData', 'setFetching']),
      ...
      /**
      * Fetch a new set of checkInTypes
      ...
      */
      fetchPaginated() {
        ...
        */
        this.$http.get(`admin/checkInTypes?page=${this.currentPage}`).then(({ data }) => {
          /**
          * Vuex action to set pagination object in
          * the Vuex Check In Type module
          */
      ...
      * return a paginated set of checkInTypes
      ...
      */
      navigate(obj) {
        ...
        */
        this.$router.push({ name: 'admin.checkInTypes.index', query: { page: obj.page } })

        /**
        * Fetch a new set of Check In Type based on
      ...
      */
      askRemove(item) {
        swal({
          title: 'Are you sure?',
          text: `CheckIn Type ${item.name} will be permanently removed.`,
          type: 'warning',
      ...
      */
      remove(checkInType) {
        this.$http.delete(`checkInTypes/${checkInType.id}`).then(() => {
          /**
          * On success fetch a new set of Check In Type
          * based on current page number
          */
          this.fetchPaginated()

          /**
          * If we remove a checkInType then
          ...
          */
          swal('Done!', 'CheckIn Type removed.', 'success')
    ...
    */
    computed: {
      ...mapState({
        ...
        pagination: state => state.Admin.CheckInTypes.pagination,
        list: state => state.Admin.CheckInTypes.list,
      }),
      checkInTypes() {
        return this.list
      },
      ...
      isFormVisible() {
        return this.$route.name === 'admin.checkInTypes.new' || this.$route.name === 'admin.checkInTypes.edit'
      },
    },
    /**
    ...
    */
    beforeRouteLeave(to, from, next) {
      ...
      this.$bus.$off('checkInType.created')
      this.$bus.$off('checkInType.updated')
    ...
    mounted() {
      ...
      */
      ...
      /**
      * CheckIn Type was created or updated, refresh the list
      */
      this.$bus.$on('checkInType.created', this.fetch)
      this.$bus.$on('checkInType.updated', this.fetch)
      /**
...
<template>
  <div class="content-wrapper">
    ...
    <!-- see /src/modules/checkInTypes/routes.js  -->
    ...
    <table class="table table-bordered table-striped" style="margin-bottom:0;">
      ...
      <tbody>
        <tr v-for="(item, index) in checkInTypes">

  ...
  export default {
    ...
    */
    ...
    methods: {
      ...
      create() {
        this.$router.push({ name: 'admin.houseTypes.new', query: { page: this.currentPage } })
      },
      hide() {
        this.$router.push({ name: 'admin.houseTypes.index', query: { page: this.currentPage } })
      },
      /**
      * Brings actions from Vuex to the scope of
      * this component
      */
      ...mapActions(['houseTypesSetData', 'setFetching']),
      ...
      */
      fetchPaginated() {
        ...
        */
        this.$http.get(`admin/houseTypes?page=${this.currentPage}`).then(({ data }) => {
          /**
          * Vuex action to set pagination object in
          * the Vuex House Types module
          */
      ...
      * return a paginated set of houseTypes
      ...
      */
      navigate(obj) {
        ...
        */
        this.$router.push({ name: 'admin.houseTypes.index', query: { page: obj.page } })

        /**
        * Fetch a new set of House Types based on
      ...
      */
      askRemove(item) {
        swal({
          title: 'Are you sure?',
          text: `House Type ${item.name} will be permanently removed.`,
          type: 'warning',
      ...
      */
      remove(houseType) {
        this.$http.delete(`houseTypes/${houseType.id}`).then(() => {
          /**
          * On success fetch a new set of House Types
          * based on current page number
          */
          this.fetchPaginated()

          /**
          * If we remove a houseType then
          ...
          */
          swal('Done!', 'House Type removed.', 'success')
    ...
    */
    computed: {
      ...mapState({
        ...
        pagination: state => state.Admin.HouseTypes.pagination,
        list: state => state.Admin.HouseTypes.list,
      }),
      houseTypes() {
        return this.list
      },
      ...
      isFormVisible() {
        return this.$route.name === 'admin.houseTypes.new' || this.$route.name === 'admin.houseTypes.edit'
      },
    },
    /**
    ...
    */
    beforeRouteLeave(to, from, next) {
      ...
      this.$bus.$off('houseType.created')
      this.$bus.$off('houseType.updated')
    ...
    mounted() {
      ...
      */
      ...
      /**
      * House Type was created or updated, refresh the list
      */
      this.$bus.$on('houseType.created', this.fetch)
      this.$bus.$on('houseType.updated', this.fetch)
      /**
...
<template>
  <div class="content-wrapper">
    ...
    <!-- see /src/modules/houseTypes/routes.js  -->
    ...
    <table class="table table-bordered table-striped" style="margin-bottom:0;">
      ...
      <tbody>
        <tr v-for="(item, index) in houseTypes">
